...
(L=20) with `susie_auto` and regular `susie` (run in two steps, first estimating residual variance, then estimating prior variance - this is because I am concerned estimating prior variance straight away may miss things, partly because at time of writing `susie_init_coef` may not be quite correct - it sets coefficients based on unstandardized X, but internally X are standardized) 
```{r}
...
compare_init = function(x,fit.tf){
  ...
  s.tf0 = susie_cp(x,s_init=s0.tf,estimate_prior_variance=FALSE)
  s.tf = susie_cp(x,s_init=s.tf0,estimate_prior_variance=TRUE)
  
  s0 = susie_cp(x,estimate_prior_variance = FALSE)
  s1 = susie_cp(x,estimate_prior_variance = TRUE)
  
  s_auto = susie_cp(x,auto=TRUE)
  return(list(fit = list(tf=s.tf,s=s1,s_auto=s_auto), obj = list(tf=get_obj(s.tf),s = get_obj(s1),s_auto= get_obj(s_auto))))
}
```
...
Follow up on chrom11. It seems that the susie auto fit works well here.
The tf initialization induces extra changepoints that don't have that good support, and susie struggles to lose these. This illustrates that
failing to lose fits can be as problematic as failing to find them...
```{r}
plot(chrom11$x)
lines(predict(c11$fit$tf))
lines(predict(c11$fit$s),col=2)
lines(predict(c11$fit$s_auto),col=3)

plot_cs(c11$fit$tf)
plot_cs(c11$fit$s)
plot_cs(c11$fit$s_auto)

lines(chrom11.res$fit$bcp$posterior.mean)
plot(chrom11.res$fit$bcp$posterior.prob)
```
